...
class Checkin(Base):
    ...
    token_id: Mapped[UUID] = mapped_column(
        ForeignKey("tokens.id"), unique=True, nullable=False
    )
    ...
    bike_photo_url: Mapped[str | None] = mapped_column(String(500))
    ...
    # Operators
    checked_in_by: Mapped[UUID | None] = mapped_column(ForeignKey("operators.id"))
    checked_out_by: Mapped[UUID | None] = mapped_column(ForeignKey("operators.id"))
    ...
    def __repr__(self) -> str:
        return (
            f"<Checkin {self.token_id} @ Rack {self.rack_id} Slot {self.slot_number}>"
        )
